...
const fetchCycleId = function (wd: wdCourseData) {
  ...
  let xhr = htmlRequest(url).then((html) => {
    const xpath = html.evaluate(
      `//usos-frame[contains(., "${wdCourseData.serializeAcademicCycle(
        wd.previousCycle()
      )}")]//td[contains(., "Typ zajęć:")]`,
      html
    );
    const item: any = xpath.iterateNext(); // bad type
    const query =
      item.nextElementSibling.lastElementChild.querySelector("a").href;
...
const fetchGroups = function (cycleId: string, teacher: string) {
  ...
  let xhr = htmlRequest(url).then((html) => {
    ...
    const xpath = html.evaluate(
      `//table[contains(., "Grupa")]//td[contains(., "${teacher}")]`,
      html
    );
    while ((item = xpath.iterateNext() as Element)) {
      ...
      let comment = item?.previousElementSibling?.textContent
        ?.trim()
        .split("\n") as Array<string>;
      ...
      groups.push(
        new usosCourseGroup(
          ...
          parseInt(
            item?.previousElementSibling?.previousElementSibling?.textContent?.trim() as string
          )
        )
      );
    }
