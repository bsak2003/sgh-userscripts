...
const createGroupTable = function (groups: Array<usosCourseGroup>) {
  ...
  const groupIdHeader = header.insertCell();
  groupIdHeader.style.border = border;
  groupIdHeader.innerText = "Nr grupy";

  const locationHeader = header.insertCell();
  locationHeader.style.border = border;
  locationHeader.innerText = "Czas i lokalizacja";

  const amountHeader = header.insertCell();
  amountHeader.style.border = border;
  amountHeader.innerText = "Liczba zapisanych";

  const limitHeader = header.insertCell();
  limitHeader.style.border = border;
  ...
  groups.forEach((x) => {
    ...
    location.style.border = border;
    location.innerText = x.comment;

    let amount = row.insertCell();
    amount.style.border = border;
    amount.innerText = x.amount.toString();

    let limit = row.insertCell();
    limit.style.border = border;
    limit.innerText = x.limit.toString();
  });

  const footer = table.insertRow();

  const groupIdFooter = footer.insertCell();
  groupIdFooter.style.border = border;
  groupIdFooter.innerText = `łącznie #${groups.length}`;

  const locationFooter = footer.insertCell();
  locationFooter.style.border = border;
  locationFooter.innerText = "czas i lokalizacja";

  const amountFooter = footer.insertCell();
  amountFooter.style.border = border;
  amountFooter.innerText = `łącznie ${groups.reduce(
    (x, y) => x + y.amount,
    0
  )}`;

  const limitFooter = footer.insertCell();
  limitFooter.style.border = border;
